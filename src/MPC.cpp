 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 1; i < N; i++) {
      ...
      AD<double> fx0      = coeffs[3] * x0 * x0 * x0+coeffs[2] * x0 * x0+coeffs[1] * x0+coeffs[0];
      ...
      fg[1 + cte_start + i]  = cte1 - ((fx0 - y0) + (v0 * CppAD::sin(epsi0) * dt));
      fg[1 + epsi_start + i] = epsi1 - ((psi0 - psides0) + v0 * delta / Lf * dt);
    }
  }
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for(int i=0; i< N; i++){
      answer.push_back(solution.x[x_start+i]);
      answer.push_back(solution.x[y_start+i]);
  }
